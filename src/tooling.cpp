...
void makeFuncAndTauNames(FunctionDecl* func, ASTContext* context,
    SourceManager& src_mgr, std::string& func_name, std::string& tau_name);
...
void make_begin_func_code(inst_loc* loc, std::string& code) {
  if (!loc->skip) {
    ...
    // needed to not get profile.-1.x.x files
    code += "#ifndef TAU_MPI\n";
...
void make_end_func_code(inst_loc* loc, std::string& code, std::string& line) {
  ...
  // types are harder, need to pull the arg to return before the stop in case it does things
    ...
    if (!loc->skip) {
      ...
      if (temp.find("return;") != std::string::npos) {
        ...
        code += "\t{TAU_PROFILE_STOP(tautimer); return;}\n";
        return;
      }
      else {
        // special case void*
...
bool check_loc_against_list(std::list<std::string> list, inst_loc* loc) {
  ...
  std::string tau_name = std::string(loc->full_tau_name).substr(0, std::string(loc->full_tau_name).find("["));
  for (std::string item : list) {
    if (matchName(item, tau_name)) {
...
bool check_func_against_list(std::list<std::string> list, FunctionDecl* func, ASTContext* context, SourceManager& src_mgr) {
  std::string func_name;
  std::string tau_name;
  // printf("checking function against list\n");
  makeFuncAndTauNames(func, context, src_mgr, func_name, tau_name);
  tau_name = tau_name.substr(0, tau_name.find("["));
  // printf("tau_name %s\n", tau_name.c_str());
  for (std::string item : list) {
    // printf("item %s\n", item.c_str());
    if (matchName(item, tau_name)) {
...
bool matchFileName(std::string& regex, std::string& match) {
  ...
  // printf("string  %s\n", match.c_str());
  // printf("regex   %s\n", regex.c_str());
  // printf("match?  %s\n", std::regex_match(trim(match), std::regex(trim(regex), std::regex_constants::egrep)) ? "Yes": "No");
...
bool check_file_against_list(std::list<std::string> list, std::string fname) {
  // printf("checking file against list\n");
  for (std::string item : list) {
...
public:
  explicit FindReturnVisitor(ASTContext* context, SourceManager& SM)
    : context(context), src_mgr(SM) {}
...
private:
  void makeRetInstLoc(ReturnStmt* retstmt) {
    ...
    std::string func_name;
    std::string tau_name;
    makeFuncAndTauNames(encl_function, context, src_mgr, func_name, tau_name);
    ...
    if (encl_function->getReturnType()->isClassType()) {
      CXXRecordDecl* decl = encl_function->getReturnType()->getAsCXXRecordDecl();
        ...
        if (!((!decl->hasUserDeclaredCopyAssignment() && !DefaultedCopyAssignmentIsDeleted) || decl->hasTrivialCopyAssignment())) {
      #endif
        needs_move = true;
      }
    }
...
public:
  ...
  bool VisitFunctionDecl(FunctionDecl *func) {
    ...
    if (func->hasBody() && (!func->isInlined() || inst_inline ||
         check_func_against_list(includelist, func, context, src_mgr))) { //
...
private:
  void makeFuncInstLoc(FunctionDecl* func) {
    ...
    //llvm::outs() << "Found function " << tau_name << "\n";
  }
};
...
class FindFunctionConsumer : public clang::ASTConsumer {
  ...
  SourceManager& src_mgr;
public:
  explicit FindFunctionConsumer(ASTContext* context, SourceManager& SM)
    : func_visitor(context, SM), src_mgr(SM) {}
  ...
  virtual void HandleTranslationUnit(ASTContext& context) {
    ...
    for (auto &decl : decls) {
      ...
      if (src_mgr.isInSystemHeader(srcloc) || src_mgr.isInExternCSystemHeader(srcloc) || src_mgr.isInSystemMacro(srcloc)) {
        continue;
      }
      // if file include list exists: *only include those*
      // except for things in the exclude list
      if (!fileincludelist.empty()) {
        if (check_file_against_list(fileincludelist, src_mgr.getFilename(srcloc).str()) && !check_file_against_list(fileexcludelist, src_mgr.getFilename(srcloc).str())) {
          // printf("adding1 %s\n", src_mgr.getFilename(srcloc).str().c_str());
          if (!src_mgr.getFilename(srcloc).str().empty() && std::find(files_to_go.begin(), files_to_go.end(), src_mgr.getFilename(srcloc).str()) == files_to_go.end()) {
...
public:
  virtual std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &Compiler, llvm::StringRef InFile) {
      return std::unique_ptr<ASTConsumer>(new FindFunctionConsumer(&Compiler.getASTContext(), Compiler.getSourceManager()));
  }
};

void instrument_file(std::ifstream& og_file, std::ofstream& inst_file, std::string filename, std::vector<inst_loc*> inst_locations, bool use_cxx_api) {
  std::string line;
  int lineno = 0;
        ...
        while (inst_loc_iter != inst_locations.end() && lineno == curr_inst_loc->line) {
          ...
          switch (curr_inst_loc->kind) {
            ...
            case RETURN_FUNC:
              if (!use_cxx_api) {
                inst_file << "\n#line " << lineno << "\n";
                make_end_func_code(curr_inst_loc, inst_code, line);
                inst_file << inst_code;
                inst_file << "#line " << lineno << "\n";
            ...
            case MULTILINE_RETURN_FUNC:
              if (!use_cxx_api) {
                inst_file << "\n#line " << lineno << "\n";
                // join all the lines together into one so make_end_func_code can actually know what's happening
...
static llvm::cl::OptionCategory MyToolCategory("TAU instrumentor options",
  "To pass options directly to the compiler, use\n\ttooling [options] <source> -- [compiler options]");

llvm::cl::opt<std::string> outputfile("tau_output",
  llvm::cl::desc("Specify name of output instrumented file"),
  llvm::cl::value_desc("filename"),
  llvm::cl::cat(MyToolCategory));

llvm::cl::opt<bool> use_cxx_api("tau_use_cxx_api",
  llvm::cl::desc("Use TAU's C++ instrumentation API"),
  llvm::cl::init(false),
  llvm::cl::cat(MyToolCategory));

llvm::cl::opt<bool> do_inline("tau_instrument_inline",
  llvm::cl::desc("Instrument inlined functions (default: false)"),
  llvm::cl::init(false),
  llvm::cl::cat(MyToolCategory));
    ...
    for (int i = 0 ; i < *argc ; i++) {
        if (strcmp(argv[i], "--") == 0) {
            ...
            if ((strncmp(argv[i], "-L", 2) != 0) &&  // ignore library paths
                ...
                (strncmp(argv[i], "-Wl", 3) != 0)) { // ignore linker options
                new_argv[new_i] = strdup(argv[i]);
                new_i++;
            } else {
                new_argc--;
...
int main(int argc, const char **argv) {
  ...
  auto ExpectedParser = tooling::CommonOptionsParser::create(new_argc, (const char**)new_argv, MyToolCategory, llvm::cl::OneOrMore,
    "Tool for adding TAU instrumentation to source files.\nNote that this will only instrument the first source file given.");
  if (!ExpectedParser) {
  ...
  // printf("after sort/unique\n");
  // dump_all_locs();

  // sort on filename excluding path
  std::sort(files_to_go.begin(), files_to_go.end(),
  ...
  auto new_end2 = std::unique(files_to_go.begin(), files_to_go.end(),
    [&](std::string s1, std::string s2) {
      // printf("s1 %s\n", s1.c_str());
      std::string temp1, temp2;
      temp1 = s1;
  ...
  for (std::string fname : files_to_go) {
    ...
    if (file_set.count(short_name) == 0) { continue; }
    ...
    // filter on fname
    for (inst_loc* loc : all_inst_locations) {
      std::string tau_name = std::string(loc->full_tau_name);
      // grab contents of first set of curly braces, which is filename
      std::string loc_fname = tau_name.substr(tau_name.find_first_of("{")+1, tau_name.find_first_of("}") - tau_name.find_first_of("{")-1);
      // printf("loc file %s\n", loc_fname.c_str());
      // printf("cur file %s\n", fname.c_str());
      ...
      //   printf("loc file %s\n", loc_fname.c_str());
      //   printf("cur file %s\n", fname.c_str());
      //   return trim(loc_fname) == trim(fname);
      // });
    ...
    instrument_file(og_file, inst_file, fname, inst_locations, use_cxx_api);
    og_file.close();
    inst_file.close();
  }
